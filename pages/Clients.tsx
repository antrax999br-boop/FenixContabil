...
const ClientsPage: React.FC<ClientsPageProps> = ({ clients, onAdd }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onAdd(newClient);
    setNewClient({ name: '', cnpj: '', interest_percent: 1.0, observations: '' });
    setShowModal(false);
  };
                ...
                <tr key={client.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button className="p-1.5 text-slate-400 hover:text-brand-orange hover:bg-brand-orange/10 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-lg text-slate-800">Cadastrar Novo Cliente</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  value={newClient.interest_percent}
                  onChange={e => setNewClient({...newClient, interest_percent: parseFloat(e.target.value)})}
                />
              </div>
              <div>
